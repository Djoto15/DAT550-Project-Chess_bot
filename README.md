...
Run the file <i>selfplay_to_pgn.py</i> in order to produce a sample game (output = sample_game.pgn i.e. game with specific parameters on the top of the file selfplay_to_pgn.py: MAX_PLIES = 60 (total nb of moves), MCTS_ITERS_PER = 30 (nb of iteration = MAX_PLIES / 2), DEPTH = 4 (depth of the tree).
Run the file <i>evaluate_vs_stockfish.py</i>i> in order to plot the success rate of Stockfish’s move being among the top five proposals generated by the bot (time consumming !).
